...
if len(sys.argv) not in (1, 3, 4):
    ...
    try:
        if Config.TG_BOT_USERNAME is not None:
            LOGS.info("𖠕 جار استرداد ملفات التيلثون العرب 𖠕")
            # ForTheGreatrerGood of beautification
            bot.tgbot = TelegramClient(
                "TG_BOT_TOKEN", api_id=Config.APP_ID, api_hash=Config.API_HASH
            ).start(bot_token=Config.TG_BOT_TOKEN)
            LOGS.info("𖠕 اكتمل تحميل البينات التيلثون بنجاح 𖠕")
            LOGS.info("𖠕 يتم بدء بوت تليثون ماركو 𖠕")
            bot.loop.run_until_complete(add_bot(Config.TG_BOT_USERNAME))
            LOGS.info("𖠕 اكتمل بدء بوت تليثون ماركو 𖠕")
        else:
            bot.start()
    except Exception as e:
...
LOGS.info("\n𖠕 @H_L_5 - اذا كنت بحاجه الى مساعده فتوجه الى 𖠕")
